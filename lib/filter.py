...
def filter_by_list(identifiers, indices, list_file, invert):
    """Filter included set using list file."""
    data = file_io.read_file(list_file)
    id_set = data.split()
    if not invert:
        indices = [i for i in indices if identifiers[i] in id_set]
    else:
...
def main():
    ...
    if args["--list"]:
        indices = filter_by_list(identifiers.values, indices, args["--list"], invert)
    if args["--json"]:
                        ...
                        fields.update(
                            {
                                "%s_cov"
                                % library: fetch_field(
                                    args["DATASET"], "%s_cov" % library, meta
                                )
                            }
                        )
                        fields.update(
