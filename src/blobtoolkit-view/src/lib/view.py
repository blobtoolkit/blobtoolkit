...
Usage:
  blobtoolkit-view [--format STRING...] [--host STRING] [--interactive]
  [--out PATH] [--param STRING...] [--ports RANGE] [--prefix STRING]
  [--preview STRING...] [--driver STRING] [--driver-log PATH]
  [--local] [--remote] [--timeout INT] [--view STRING...] 
  [-h|--help] [-v|--version] DIRECTORY
...
def cli(rename=None):
    """Entry point."""
    if sys.argv[0].endswith("-view"):
        args = docopt(__doc__, version=__version__)
    else:
        docs = __doc__
        if rename is not None:
            docs = docs.replace("blobtoolkit-view", rename)
